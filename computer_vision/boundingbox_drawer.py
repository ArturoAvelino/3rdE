...
class BoundingBoxDrawer:
    """
    A comprehensive Python class for drawing bounding boxes on images with
    annotation data from JSON files.

    NOTES
        COCO format.
        The first two numbers in the output JSON file with COCO format of the
        bounding box information (see section "bbox" for each object) should correspond
        ...
        ROBO format.
        The JSON file generated by robo, the first two numbers correspond to the
        bounding-box *center*, so this code read the JSON file assuming that
        it is the case.


    OVERVIEW:
    =========
    The BoundingBoxDrawer class is a powerful tool for visualizing object
    detection results by overlaying bounding boxes, labels, and metadata onto
    images. It supports two major annotation formats (COCO and Roboflow)
    and provides extensive customization options for appearance, filtering,
...
class COCOProcessor:
        ...
        for annotation in annotations:
            try:
                ...
                if 'center_x' in annotation and 'center_y' in annotation:
                    ...
                    #old # but we'll handle both cases - check if it's center or corner based on values
                    #old center_x = x
                    #old center_y = y
                    #old # If the values seem to be corner coordinates, calculate center
                    #old if x < width and y < height:  # Likely corner coordinates
                    #old     center_x = x + width / 2
                    #old     center_y = y + height / 2
    ...
    #old def extract_bbox_data(self, json_data: Dict, image_size: Tuple[int, int],
    #old                       show_id: bool = True, show_confidence: bool = True) -> \
    #old List[Dict]:
    #old     """
    #old     Extract bounding box data from COCO format JSON.
    #old
    #old     Args:
    #old         json_data (Dict): Loaded JSON data in COCO format
    #old         image_size (Tuple[int, int]): (width, height) of the image
    #old         show_id (bool): Whether to include ID information
    #old         show_confidence (bool): Whether to include confidence information
    #old
    #old     Returns:
    #old         List[Dict]: List of bounding box data dictionaries
    #old     """
    #old     bbox_data = []
    #old
    #old     # Create category mapping
    #old     category_mapping = self._create_category_mapping(json_data)
    #old
    #old     # Extract image information
    #old     image_info = self._get_image_info(json_data)
    #old     if not image_info:
    #old         return bbox_data
    #old
    #old     # Process annotations
    #old     annotations = json_data.get('annotations', [])
    #old
    #old     for annotation in annotations:
    #old         try:
    #old             # Extract bbox coordinates (COCO format: [x, y, width, height])
    #old             bbox = annotation.get('bbox', [])
    #old             if len(bbox) != 4:
    #old                 continue
    #old
    #old             x, y, width, height = bbox
    #old
    #old             # Convert to corner coordinates
    #old             x1 = x - (width / 2)
    #old             y1 = y - (height / 2)
    #old             x2 = x1 + width
    #old             y2 = y1 + height
    #old
    #old             # Calculate center from original COCO data (if available) or from bounding box
    #old             if 'center_x' in annotation and 'center_y' in annotation:
    #old                 center_x = annotation['center_x']
    #old                 center_y = annotation['center_y']
    #old             else:
    #old                 # For COCO format, x and y are typically top-left corner
    #old                 # but we'll handle both cases - check if it's center or corner based on values
    #old                 center_x = x
    #old                 center_y = y
    #old                 # If the values seem to be corner coordinates, calculate center
    ...
    #old                     center_y = y + height / 2
    #old
    #old             # Get category information
    #old             category_id = annotation.get('category_id')
    #old             category_name = category_mapping.get(category_id,
    #old                                                  f"Unknown_{category_id}")
    #old
    #old             bbox_dict = {
    #old                 'coordinates': (x1, y1, x2, y2),
    #old                 'label': category_name,
    #old                 'center_x': center_x,
    #old                 'center_y': center_y
    #old             }
    #old
    #old             # Add ID if requested
    #old             if show_id:
    #old                 annotation_id = annotation.get('id', 'N/A')
    #old                 bbox_dict['id'] = str(annotation_id)
    #old
    #old             # COCO format doesn't typically have confidence scores
    #old             # But if they exist in the annotation, include them if requested
    #old             if show_confidence and 'score' in annotation:
    #old                 confidence_percent = int(
    #old                     round(annotation['score'] * 100, 0))
    #old                 bbox_dict['confidence'] = f"{confidence_percent}%"
    #old
    #old             bbox_data.append(bbox_dict)
    #old
    #old         except Exception as e:
    #old             logging.getLogger(__name__).warning(
    #old                 f"Error processing annotation: {e}")
    #old             continue
    #old
    #old     return bbox_data
    #old
